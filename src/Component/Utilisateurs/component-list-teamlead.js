...
export default function CustomPaginationActionsTable(props) {
    const rows = props.listFreeTeamLead
    const classes = useStyles1();
    const [page, setPage] = React.useState(0);
    const [rowsPerPage, setRowsPerPage] = React.useState(4);
    const emptyRows = rowsPerPage - Math.min(rowsPerPage, rows.length - page * rowsPerPage);

    const [teamleadSelected, setTeamLeadSelected] = React.useState([]);
    ...
    const teamleadChecked = (e) => {
        if (e.target.checked) {
            const tabs = teamleadSelected
            tabs.push(JSON.parse(e.target.value))
            setTeamLeadSelected(tabs)
            props.teamleadSelected(teamleadSelected)
        } else {
            const tabs = teamleadSelected
    ...
    return (
        <>
            ...
            <TableContainer className="tableContainerStyles" component={Paper} style={{ width: "100%" }}>
                    ...
                    <TableBody>
                        {(rowsPerPage > 0
                            ...
                            <TableRow key={index}>
                                ...
                                <TableCell>
                                    <input value={JSON.stringify(row)} onChange={teamleadChecked} className={classes.checkBox} type="checkBox" name="actionRadio" />
                                </TableCell>
                            </TableRow>
                        ))}
