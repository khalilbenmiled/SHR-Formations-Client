...
export default function CustomPaginationActionsTable(props) {
    const rows = props.domaines
    const classes = useStyles1();
    const [page, setPage] = React.useState(0);
    const [rowsPerPage, setRowsPerPage] = React.useState(4);
    const emptyRows = rowsPerPage - Math.min(rowsPerPage, rows.length - page * rowsPerPage);
    const [listDomaines, setListDomaines] = React.useState([]);
    ...
    const domaineChecked = (e) => {
        if (e.target.checked) {
            const tabs = listDomaines
            tabs.push(JSON.parse(e.target.value))
            setListDomaines(tabs)
            props.getListDomaines(listDomaines)
        } else {
            const tabs = listDomaines
                    ...
                    <TableBody>
                        {(rowsPerPage > 0
                            ...
                            <TableRow key={index}>
                                ...
                                <TableCell>
                                    <input value={JSON.stringify(row)} onChange={domaineChecked} className={classes.checkBox} type="checkBox" name="actionRadio" />
                                </TableCell>
                            </TableRow>
                        ))}
